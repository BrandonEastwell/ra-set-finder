...
async function getActiveTabFromLocalStorage() {
  ...
  return res.ActiveTab.isValidTab ? { isValidTab: res.ActiveTab.isValidTab, tabId: res.ActiveTab.tabId, eventId: res.ActiveTab.eventId }
...
async function getEventsCacheFromLocalStorage(eventId: number) {
  ...
  if (res.Events) {
    const cachedEventFound: EventsCacheItem | undefined = res.Events.find((item: EventsCacheItem) => item.eventId === eventId)
