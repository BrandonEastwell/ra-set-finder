...
export default function ArtistList({ artists, eventId }: { artists: Artist[]; eventId: number }) {
  ...
  return (
    <div className="flex flex-col max-h-[400px] bg-black pb-0">
      <div
        className="flex flex-row items-center cursor-pointer hover:bg-purered/10 px-6 py-4"
        onClick={() => setToggleList(prevState => !prevState)}>
        <svg
          className={`w-[24px] group-hover:fill-purered transition-transform duration-100 ${toggleList ? 'rotate-90' : '-rotate-90'}`}
        ...
        <p className="text-slatewhite text-4xl cursor-pointer">Sets</p>
      </div>
      {toggleList && artists && (
